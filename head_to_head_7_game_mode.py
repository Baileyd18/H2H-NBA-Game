# H2H MODULE VERSION: room_invite_challenge_sync_after_lock_v8
...
H2H_CAP_OPTIONS = {
    "Salary Cap ($165M)": 165_000_000,
    "Salary Floor ($149M)": 149_000_000,
    "Luxury Tax ($201M)": 201_000_000,
    "First Apron ($209M)": 209_000_000,
    "Second Apron ($222M)": 222_000_000,
    "No Restrictions ($400M)": 400_000_000,
    "Custom": None,
}
...
def _roster_payroll(roster: Any) -> float:
    return sum(_num(p, "Salary", default=0.0) for p in _safe_roster(roster))


def _room_hard_cap(room: Dict[str, Any], fallback_cap: int) -> int:
    # h2h_salary_cap is the creator-selected hard cap. salary_cap is kept as
    # a fallback for older rooms created before this feature existed.
    try:
        return int(room.get("h2h_salary_cap") or room.get("salary_cap") or fallback_cap)
    except Exception:
        return int(fallback_cap)


def _cap_room_label(cap: float) -> str:
    return f"{_money(cap)} hard cap"


def _get_query_param(name: str, default: str = "") -> str:
    """Return a single Streamlit query-param value safely across Streamlit versions."""
    try:
...
def _app_base_url() -> str:
    """Base URL used for full invite links. Set APP_URL in Streamlit secrets."""
    try:
...
def _room_invite_link(room_code: str) -> str:
    base = _app_base_url()
    suffix = f"?room={str(room_code).upper().strip()}"
    return f"{base}/{suffix}" if base else suffix


def _challenge_invite_link(challenge_code: str) -> str:
    base = _app_base_url()
    suffix = f"?challenge={challenge_code}"
    return f"{base}/{suffix}" if base else suffix


def _copy_link_box(label: str, link: str, caption: str = "") -> None:
    st.markdown(f"**{label}**")
...
def _upsert_player_v2(
    ...
    room["h2h_salary_cap"] = int(salary_cap)
    room["updated_at"] = _now()
    if room.get("p1_roster") and room.get("p2_roster"):
        room["status"] = "ready"
...
def _render_room_header(room: Dict[str, Any], role: Optional[str], room_code: str, salary_cap: int) -> None:
    ...
    hard_cap = _room_hard_cap(room, salary_cap)
    st.markdown(f"### Room Code: `{room_code}`")
    st.caption(f"You are locked in as **{role_text}** • H2H Salary Limit: **{_money(hard_cap)}**")
...
def render_head_to_head_mode(
    ...
    challenge_code = _get_query_param("challenge", "")
    if challenge_code and not st.session_state.h2h_room_code:
        try:
            ...
            rooms[code] = {
                ...
                "p1_locked": True,
                "p2_locked": False,
                "salary_cap": int(payload.get("h2h_salary_cap", payload.get("salary_cap", salary_cap))),
                "h2h_salary_cap": int(payload.get("h2h_salary_cap", payload.get("salary_cap", salary_cap))),
                "h2h_salary_cap_label": payload.get("h2h_salary_cap_label", "Challenge Cap"),
            }
            _save_rooms(rooms)
    ...
    # Room invite support: opening ?room=ABC123 automatically joins as Player 2.
    # Challenge links above take priority because they create a room with Player 1's roster already loaded.
    room_from_url = _get_query_param("room", "").upper().strip()
    if room_from_url and not challenge_code and not st.session_state.h2h_room_code:
        rooms = _load_rooms()
        if room_from_url not in rooms:
            st.warning("Invite link room was not found. The host may need to create a new room.")
        else:
            room = rooms[room_from_url]
            if room.get("p1_session") == session_id:
                st.session_state.h2h_room_code = room_from_url
                st.session_state.h2h_slot = "p1"
                st.info(f"Opened your own room invite. You are Player 1 in room {room_from_url}.")
                st.rerun()
            elif room.get("p2_session") and room.get("p2_session") != session_id:
                st.session_state.h2h_room_code = room_from_url
                st.session_state.h2h_slot = "spectator"
                st.warning("This invite room already has a Player 2, so you are viewing as a spectator.")
                st.rerun()
            else:
                ...
                room["updated_at"] = _now()
                rooms[room_from_url] = room
                _save_rooms(rooms)
                st.session_state.h2h_room_code = room_from_url
                st.session_state.h2h_slot = "p2"
                st.success(f"Invite link loaded. You joined room {room_from_url} as Player 2.")
                st.rerun()
    ...
    with setup_col1:
        ...
        cap_choice = st.selectbox(
            "H2H Salary Limit",
            list(H2H_CAP_OPTIONS.keys()),
            index=list(H2H_CAP_OPTIONS.keys()).index("Second Apron ($222M)"),
            key="h2h_create_cap_choice",
            help="The room creator sets the hard cap for both teams. Player 2 cannot change it."
        )
        if cap_choice == "Custom":
            selected_h2h_cap = int(st.number_input(
                "Custom H2H Salary Limit",
                min_value=100_000_000,
                max_value=500_000_000,
                value=int(salary_cap),
                step=5_000_000,
                format="%d",
                key="h2h_custom_salary_cap"
            ))
        else:
            selected_h2h_cap = int(H2H_CAP_OPTIONS[cap_choice])

        st.caption(f"Room hard cap: {_money(selected_h2h_cap)}")
        ...
        if st.button("Create New Room", use_container_width=True, key="h2h_create_room_v2"):
            ...
            rooms[code] = {
                ...
                "p1_locked": False,
                "p2_locked": False,
                "salary_cap": int(selected_h2h_cap),
                "h2h_salary_cap": int(selected_h2h_cap),
                "h2h_salary_cap_label": cap_choice,
            }
            _save_rooms(rooms)
            st.session_state.h2h_room_code = code
            st.session_state.h2h_slot = "p1"
    ...
    with st.expander("Shareable challenge link", expanded=False):
        if enough_players:
            payload = {
                ...
                "roster": current,
                "salary_cap": selected_h2h_cap,
                "h2h_salary_cap": selected_h2h_cap,
                "h2h_salary_cap_label": cap_choice,
                "session": session_id,
            }
            challenge = encode_roster_payload(payload)
            challenge_link = _challenge_invite_link(challenge)
            _copy_link_box(
                "Challenge Mode Link",
                challenge_link,
                "Send this after building a roster. Your roster loads as Player 1, and the opponent auto-joins as Player 2."
            )
        else:
    ...
    if role == "p1":
        with st.expander("Copy room invite link", expanded=True):
            invite_link = _room_invite_link(room_code)
            _copy_link_box(
                "Room Invite Mode Link",
                invite_link,
                "Send this before or during roster building. The next user who opens it auto-joins as Player 2."
            )
    elif role == "p2":
        st.caption("You joined this room as Player 2 from the invite/code. Build your roster, then lock it when ready.")

    # Smart room sync.
    # Auto-refresh intentionally stays OFF while either player is still drafting.
    # Streamlit reruns close player-selection dialogs, so sync only starts after
    # both rosters are locked. It then keeps both players updated through the
    # live series and completed-series storyline waiting, and stops once the
    # saved AI storyline exists.
    both_rosters_locked = (
        bool(room.get("p1_locked"))
        and bool(room.get("p2_locked"))
        and bool(room.get("p1_roster"))
        and bool(room.get("p2_roster"))
    )
    storyline_saved = bool(str(room.get("storyline", "")).strip())
    auto_sync_active = both_rosters_locked and not storyline_saved

    sync_col1, sync_col2 = st.columns([0.55, 0.45])
    with sync_col1:
        live_sync = st.toggle(
            "Auto-refresh room every 3 seconds",
            value=True,
            key="h2h_live_sync_v6",
            help="Starts only after both rosters are locked, then stops after Player 1 generates the AI storyline."
        )

        if live_sync and auto_sync_active:
            if st_autorefresh is not None:
                st.caption("Live room sync is active. It will stop after the AI storyline is generated.")
                st_autorefresh(interval=3000, key=f"h2h_room_sync_{room_code}")
            else:
                st.warning(
                    "Auto-refresh requires streamlit-autorefresh. Add it to requirements.txt, "
                    "then redeploy. Manual Refresh still works."
                )
        elif live_sync and storyline_saved:
            st.caption("Auto-refresh stopped because the AI storyline has been generated.")
        elif live_sync:
            st.caption("Auto-refresh will start once both teams have locked their rosters.")
        else:
    ...
    if role in {"p1", "p2"}:
        hard_cap = _room_hard_cap(room, salary_cap)
        current_payroll = _roster_payroll(current)
        remaining_h2h_cap = hard_cap - current_payroll

        cap_a, cap_b, cap_c = st.columns(3)
        cap_a.metric("H2H Salary Limit", _money(hard_cap))
        cap_b.metric("Your Payroll", _money(current_payroll))
        cap_c.metric("Cap Room", _money(remaining_h2h_cap), delta=None if remaining_h2h_cap >= 0 else f"Over by {_money(abs(remaining_h2h_cap))}")

        over_h2h_cap = current_payroll > hard_cap
        if over_h2h_cap:
            st.error(
                f"Your roster payroll is {_money(current_payroll)}, which is "
                f"{_money(current_payroll - hard_cap)} over the H2H salary limit of {_money(hard_cap)}. "
                "Remove salary before locking your roster for this matchup."
            )
        else:
            st.success(f"Your roster is under the H2H salary limit with {_money(remaining_h2h_cap)} remaining.")
        ...
        if not enough_players:
            ...
            lock_disabled = own_locked or over_h2h_cap
            if st.button(label, type="primary", use_container_width=True, key="h2h_lock_roster_v2", disabled=lock_disabled):
                _upsert_player_v2(
                    room_code,
                    role,
