# H2H MODULE VERSION: role_locked_team_name_only_v4_manual_sync
...
def render_head_to_head_mode(
    ...
    # Manual sync only.
    # Do NOT use meta-refresh here because it reloads the entire browser page on
    # Streamlit Cloud and can reset each user's app session.
    sync_col1, sync_col2 = st.columns([0.55, 0.45])
    with sync_col1:
        st.caption("Sync mode: manual refresh. Player 2 will see Player 1 updates after tapping Refresh Room.")
    with sync_col2:
