# H2H MODULE VERSION: room_invite_and_challenge_mode_v7
...
def render_head_to_head_mode(
    ...
    # Smart room sync.
    # Starts after Player 2 joins the room, keeps both players updated through
    # roster lock + live series + completed-series storyline waiting, and stops
    # only after the saved AI storyline exists. This avoids destructive browser
    # meta-refresh while keeping the H2H room feeling live.
    p2_has_joined = bool(room.get("p2_session"))
    storyline_saved = bool(str(room.get("storyline", "")).strip())
    auto_sync_active = p2_has_joined and not storyline_saved
    ...
    with sync_col1:
        live_sync = st.toggle(
            ...
            value=True,
            key="h2h_live_sync_v5",
            help="Starts once Player 2 joins and stops after Player 1 generates the AI storyline."
        )
        ...
        if live_sync and auto_sync_active:
            ...
            st.caption("Auto-refresh will start once Player 2 joins the room.")
        else:
