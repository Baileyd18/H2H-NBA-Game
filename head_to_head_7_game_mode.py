...
def render_head_to_head_mode(
    ...
    st.caption("Create a challenge, lock rosters, simulate a best-of-seven series, then generate box scores and AI storylines.")
    ...
    if "h2h_name" not in st.session_state:
        st.session_state.h2h_name = ""

    # Optional URL challenge payload support.
    ...
    c1, c2 = st.columns(2)
    with c1:
        player_name = st.text_input("Your display name", value=st.session_state.h2h_name, placeholder="Player 1")
        st.session_state.h2h_name = player_name
        if st.button("Create New Room", use_container_width=True, key="h2h_create_room"):
            code = _make_room_code()
            st.session_state.h2h_room_code = code
            st.session_state.h2h_slot = "p1"
            rooms = _load_rooms()
            rooms[code] = {
                ...
                "updated_at": _now(),
                "status": "waiting",
                "p1_name": player_name or "Player 1",
                "p2_name": "Player 2",
                "p1_roster": [],
                "p2_roster": [],
            ...
            _save_rooms(rooms)
            st.success(f"Room created: {code}")
            st.rerun()

    with c2:
        join_code = st.text_input("Room code", value=st.session_state.h2h_room_code, max_chars=6, key="h2h_join_code").upper().strip()
        slot = st.radio("Join as", ["p1", "p2"], format_func=lambda x: "Player 1" if x == "p1" else "Player 2", horizontal=True, key="h2h_join_slot")
        if st.button("Join Room", use_container_width=True, key="h2h_join_room"):
            rooms = _load_rooms()
            if join_code in rooms:
                st.session_state.h2h_room_code = join_code
                st.session_state.h2h_slot = slot
                st.success(f"Joined room {join_code}.")
                st.rerun()
            else:
    ...
    with st.expander("Shareable challenge link", expanded=False):
        if enough_players:
            payload = {"name": player_name or "Player 1", "roster": current, "salary_cap": salary_cap}
            challenge = encode_roster_payload(payload)
            st.code(f"?challenge={challenge}", language="text")
            st.caption("Send this URL query string to another player. When they open it, your roster loads as Player 1 and they build as Player 2.")
        else:
    ...
    st.caption(f"Salary Cap: {_money(room.get('salary_cap', salary_cap))}")

    if not enough_players:
        st.warning(f"Draft at least {MIN_H2H_PLAYERS} players before locking your roster. Current: {len(current)}")
    else:
        label = f"Lock Current Roster as {'Player 1' if st.session_state.h2h_slot == 'p1' else 'Player 2'}"
        if st.button(label, type="primary", use_container_width=True, key="h2h_lock_roster"):
            _upsert_player(room_code, st.session_state.h2h_slot, player_name, current, salary_cap)
            st.success("Roster locked.")
            st.rerun()

    a, b = st.columns(2)
    with a:
        _render_roster_cards(room.get("p1_name", "Player 1"), room.get("p1_roster", []))
    with b:
        _render_roster_cards(room.get("p2_name", "Player 2"), room.get("p2_roster", []))

    ready = bool(room.get("p1_roster")) and bool(room.get("p2_roster"))
    if not ready:
        st.info("Waiting for both players to lock rosters.")
        if st.button("Refresh Room", use_container_width=True, key="h2h_refresh"):
            st.rerun()
        return

    sim1, sim2 = st.columns([0.65, 0.35])
    with sim1:
        injuries_enabled = st.toggle("Enable injuries / fatigue storylines", value=True, key="h2h_injuries")
    with sim2:
        if st.button("Simulate New 7-Game Series", type="primary", use_container_width=True, key="h2h_simulate"):
    ...
    if not result:
        st.info("Both rosters are ready. Simulate the series when you’re set.")
        return
    ...
    st.markdown(f"## {winner_name} wins the series {result['p1_wins']}-{result['p2_wins']}")
    m1, m2, m3 = st.columns(3)
    m1.metric(p1_name, f"{result['p1_wins']} wins", f"OVR {result['p1_strength']['overall']}")
    m2.metric(p2_name, f"{result['p2_wins']} wins", f"OVR {result['p2_strength']['overall']}")
    ...
    with st.expander("Game-by-game box scores", expanded=True):
        ...
        selected_label = st.selectbox("Choose game", game_labels, key="h2h_box_game")
    ...
    if st.button("Generate AI Storyline", use_container_width=True, key="h2h_storyline"):
        story = generate_ai_series_storyline(result, room)
        rooms = _load_rooms()
    ...
    if room.get("storyline"):
        ...
        st.caption("Generate an AI storyline after the series to get a playoff-style recap.")

    if st.button("Reset This Room Result", use_container_width=True, key="h2h_reset_result"):
        rooms = _load_rooms()
