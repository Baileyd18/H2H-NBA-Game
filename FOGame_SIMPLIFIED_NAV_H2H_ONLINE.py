...
try:
    from head_to_head_7_game_mode import render_head_to_head_mode
except Exception:
...
def render_suggested_players_row(slot: str, suggested_players: pd.DataFrame):
    """
    Native Streamlit suggested-player row.

    Why this version:
    - Uses real st.button widgets, so Add works inside Streamlit dialogs.
    - Uses CSS to keep the 5 suggestion columns in one horizontal scroll row on mobile.
    - Avoids the old JavaScript/query-param click approach that was failing in the Build tab.
    """
    ...
    st.markdown(
        """
        <style>
        /* Native suggested-card row: horizontal swipe, no stacking, no cramping */
        .suggest-native-marker {
            display: none !important;
        }

        div[data-testid="stHorizontalBlock"]:has(.suggest-native-marker) {
        ...
        div[data-testid="stHorizontalBlock"]:has(.suggest-native-marker)::-webkit-scrollbar {
            height: 6px;
        }

        div[data-testid="stHorizontalBlock"]:has(.suggest-native-marker)::-webkit-scrollbar-thumb {
            background: rgba(56, 189, 248, .38);
            border-radius: 999px;
        }

        div[data-testid="stHorizontalBlock"]:has(.suggest-native-marker) > div[data-testid="column"] {
        ...
        .suggest-native-card {
            ...
            background:
                radial-gradient(circle at top, rgba(14,165,233,.20), transparent 45%),
        ...
        .suggest-native-card .suggest-pos {
            display: inline-block;
            color: #38bdf8;
            border: 1px solid rgba(56,189,248,.55);
            background: rgba(14,165,233,.10);
        ...
        .suggest-native-card .suggest-img-wrap img,
        .suggest-native-card .suggest-img-wrap .avatar-circle {
        ...
        .suggest-native-card .suggest-name {
            color: #f8fafc;
            font-size: 12px;
            line-height: 1.12;
            font-weight: 950;
        ...
        .suggest-native-card .suggest-meta {
            color: #94a3b8;
            font-size: 10px;
            margin-top: 6px;
            white-space: nowrap;
        ...
        .suggest-native-card .suggest-salary {
            color: #38bdf8;
            font-size: 13px;
            font-weight: 950;
            margin-top: 6px;
        ...
        div[data-testid="stHorizontalBlock"]:has(.suggest-native-marker) .stButton > button {
            min-height: 34px !important;
    ...
    for i, (_, row) in enumerate(shown.iterrows()):
        with cols[i]:
            if i == 0:
                st.markdown('<span class="suggest-native-marker"></span>', unsafe_allow_html=True)

            st.markdown(suggested_player_card_html(row, slot), unsafe_allow_html=True)

            player_name = str(row.get("Player", f"player_{i}"))
            safe_key_name = normalize_name_for_match(player_name).replace(" ", "_")
            if st.button(
                "Add",
                key=f"suggest_native_add_{slot}_{i}_{safe_key_name}",
...
def app_h2h_team_strength(roster_list):
    ...
    try:
        ...
        from head_to_head_7_game_mode import default_team_strength
